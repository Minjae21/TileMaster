        ...
        if(key === 'Enter') {
            if(state.currentCol === 5) { // check if all cols are filled
                ...
                if(isWordValid(word)) {
                    ...
                    remainChance();
                    if (state.currentRow === 9 && state.secret !== word) {
...
function revealWord(guess) {
    ...
    const isGameOver = state.currentRow === 9 && !isWinner; // game over when the row reaches 9 and not winner
    ...
    setTimeout(() => {
        if(isWinner) {
            ...
            alert(`No remaining chances! The answer was ${state.secret}.`);
            stopGame();
        }
